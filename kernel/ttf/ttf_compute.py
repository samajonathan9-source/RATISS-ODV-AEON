...
class TTFBrain:
    ...
    # ── Couche Q réelle (t-J Lanczos) via la brique existante ──
    def quantum_layer(self, Lx: int = 4, Ly: int = 4, t: float = 1.0, J: float = 0.3) -> dict:
        from kernel.solvers.quantum_solver import solve_quantum_hybrid
        res = solve_quantum_hybrid({"Lx": Lx, "Ly": Ly, "t": t, "J": J, "qubits": min(8, Lx * Ly)})
        self.t_j_res = res
        return res

    # ── Preuve ZK du chemin TSP (gluon d'info) via la brique existante ──
    def _zk_proof(self, tsp: dict) -> dict:
        ...
        return generate_risc_zero_proof(payload)
